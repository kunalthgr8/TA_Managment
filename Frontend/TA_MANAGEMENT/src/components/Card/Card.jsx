...
function Card({ width = "100px", height = "100px", className = "", src = "" }) {
  return (
    <div
      className={`flex flex-col md:flex-row justify-between bg-slate-100 rounded-lg p-5 gap-4 ${className}`}
    >
      <div className="flex flex-col justify-center self-center w-full  md:flex-row  gap-5 md:justify-evenly p-4">
        <img
          className="rounded-full flex justify-center self-center"
          src={src}
          alt="Profile"
          style={{ width, height }}
        />
      ...
      <Button
        width="flex justify-center self-center"
        className="bg-gray-300 w-32 font-bold rounded-3xl text-custom-purple"
      >
        Details
      </Button>
    </div>
